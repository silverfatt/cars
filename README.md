...
- Составление датасет, были проанализированы данные по таксопаркам в России:
	1. Какие модели машины чаще всего используются;
	2. С какого года выпуска автомобиль может таксовать;
	3. Средний пробег для данных машин

-  Генирация дат последнего автомобиля, так чтобы были авто с пройдённым ТО, а также с просроченным.
-  Для предсказания рекомендованной даты следующего обслуживания был выбран способ регрессии:
	1. Прогноз является числовым и непрерывным;
	2. Дата обслуживания может быть рассчитана на основе этого значения, прибавив прогнозируемое кол-во дней к текущей дате.
	3. Использовалось: RandomForestRegressor
	4. Выбор признаков: year_of_manufacrute, mileage
	5. Целевая переменна: days_to_next_to (следующая дата ТО)
	6. Для оценки качества использовались: 
...
- Помимо изначальных условия для обучения, дополнить датасета новыми признаками (состояние двигателя, на каком топливе авто и тд);
- Заменить базовую модель на более сложную (XGBoost)
- Применение ансамблевых методов для повышения стабильности
